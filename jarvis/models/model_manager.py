...
class ModelManager:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info("✅ OpenAI GPT-4o-mini initialized (primary)")
        except Exception as e:
            logger.warning(f"⚠️ OpenAI model initialization failed: {str(e)}")
        ...
        try:
            ...
            if self.ollama_model.is_available():
                ...
                logger.info(f"✅ Ollama {self.ollama_model.model_name} initialized (fallback)")
            else:
                logger.warning("⚠️ Ollama initialized but not responding")
                self.ollama_model = None
        except Exception as e:
            logger.warning(f"⚠️ Ollama model initialization failed: {str(e)}")
            self.ollama_model = None
        ...
        if self.ollama_available:
            try:
                ...
                if not response.startswith("Error:"):
                    logger.info("✅ Response generated by Ollama (fallback)")
                    return response
                else:
                    logger.error(f"❌ Ollama failed: {response}")
